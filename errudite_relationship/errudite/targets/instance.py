...
class Instance(object):
    ...
    def get_perform(self, model: str=None, perform_name: str=None) -> float:
        ...
        try:
            ...
            if all(prediction[i].get_perform(perform_name) == 1 for i in range(3)):
                return 1
            else :
                return 0
        except:
    ...
    def is_incorrect(self, model: str=None) -> bool:
        ...
        try:
            prediction = self.get_entry('prediction', model)
            if all(not prediction[i].is_incorrect() for i in range(3)):
                return False
    ...
    def get_entry(self, entry: str, model: str=None) -> Target:
        ...
        if entry in self.entries:
            ...
            predictions = []
            prediction_PE = getattr(self, 'prediction_PE', [])
            prediction_KE = getattr(self, 'prediction_KE', [])
            prediction_LCE = getattr(self, 'prediction_LCE', [])
            predictions.append(prediction_PE)
            predictions.append(prediction_KE)
            predictions.append(prediction_LCE)
